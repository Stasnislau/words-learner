...
const Question = ({ question }: { question: string }) => {
    ...
    return (
        <motion.div
            className="flex flex-row w-fit bg-white justify-center rounded-md px-20 py-4 z-[1]"
            initial={{ opacity: 0.3, position: "absolute", transform: "translate(-50%, -50%) scale(1.5)", top: "50%", left: "50%" }}
            animate={isQuestionVisible ? { opacity: 1, top: "10%", transform: "scale(1) translate(-50%, -50%)" } : { opacity: 1 }}
